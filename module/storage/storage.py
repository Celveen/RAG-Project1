...
# Refer to https://github.com/momount/nano-graphrag/blob/main/examples/using_faiss_as_vextorDB.py#L19
@dataclass
class FaissVectorDBStorage(BaseVectorStorage):
    ...
    def __post_init__(self):
        self._index_file_name = os.path.join(
            self.save_dir, f"vdb_{self.namespace}.index"
        )
        self._metadata_file_name = os.path.join(
            self.save_dir, f"vdb_{self.namespace}_metadata.pkl"
        )

        self._max_batch_size = self.embedding_batch_num
    ...
    async def upsert(self, data: Dict[str, dict]):
        ...
        embeddings_list = await tqdm_asyncio.gather(
        ...
        for k, v in data.items():
            # Map original_id(str) into xxhash_id(integer)
    ...
    async def query(self, query: str, top_k=5, query_embedding_dict: dict = {"query_id": None, "embedding_path": None}):
        ...
        for distance, idx in zip(distances[0], indices[0]):
            ...
            if idx != -1:  # FAISS returns -1 for empty slots
                if idx in self._metadata:
                    ...
                    # ** 表示创建一个新的字典，其中包含 metadata 字典中的所有键值对，再加上一个键为 "distance"，值为 1 - distance 的新的键值对。
...
class NanoVectorDBStorage(BaseVectorStorage):
    ...
    def __post_init__(self):
        self._client_file_name = os.path.join(
            self.save_dir, f"vdb_{self.namespace}.json"
        )
...
class NetworkXStorage(BaseGraphStorage):
    ...
    def __post_init__(self):
        self._graphml_xml_file = os.path.join(
            self.save_dir, f"graph_{self.namespace}.graphml"
        )
